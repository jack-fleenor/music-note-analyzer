...
#   file_length=file.getnframes() 
#   f_s=file.getframerate() #sampling frequency
#   sound = np.zeros(file_length) #blank array
#   for i in range(file_length) : 
#     frames = file.getparams().sampwidth * file.getparams().nchannels 
#     options = { 2: "<h", 4: "<l"}
#     wdata=file.readframes(1)
#     data=unpack(options[frames], wdata) # h -- short (2bytes) l -- long (4bytes)
#     sound[i] = int(data[0])
#   sound=np.divide(sound,float(2**15)) #scaling it to 0 - 1
#   counter = file.getnchannels() #number of channels mono/sterio
#   #fourier transformation from numpy module
